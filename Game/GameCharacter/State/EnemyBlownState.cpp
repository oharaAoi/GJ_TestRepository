...
void EnemyBlownState::IsBlown() {
	...
	acceleration *= std::powf(dampingFactor_, kDeltaTime);
	...
	velocity *= std::powf(dampingFactor_, kDeltaTime);

	velocity.y = 0;
	acceleration.y = 0;

	enemy_->SetAcceleration(acceleration);
	enemy_->SetVelocity(velocity + (enemy_->GetAcceleration() * kDeltaTime));
}

void EnemyBlownState::IsFalling() {
	Vector3 acceleration = { 0,kGravity_,0 };
	enemy_->SetAcceleration(acceleration);
	enemy_->SetVelocity(enemy_->GetVelocity() + (enemy_->GetAcceleration() * kDeltaTime));
}
